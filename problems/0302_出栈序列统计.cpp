/*
0302: 出栈序列统计
http://dsb.openjudge.cn/practice3/0302/
...
using namespace std;
int n=0;
int function(int wait,int in,int pop)
{
	int number=0;
	if(pop==n)
		return 1;
	if(wait>0)
	{
		number+=function(wait-1,in+1,pop);
	}
	if(in>0)
	{
		number+=function(wait,in-1,pop+1);
	}
	//cout<<"wait: "<<wait<<"in: "<<in<<"pop: "<<pop<<"number:"<<number<<endl;
	return number;
}
int main()
{
